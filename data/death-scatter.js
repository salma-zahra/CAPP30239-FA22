...
d3.csv('everest-deaths.csv').then(data => {

    console.log(CLIMBER.length)
  
  let x = d3.scaleLinear()
    .domain(d3.extent(data, d => d.MYEAR)).nice()
  ...
  svg.append("g")
      ...
      .attr("fill", "orange")
      .attr("opacity", 0.75)
      .attr("transform", d => `translate(${x(d.MYEAR)},${y(d.flipper_length_mm)})`);
  ...
  d3.selectAll(".cross") // note we are selecting all the symbols by class
    .on("mouseover", function(event, d) {
      ...
      tooltip
        ...
        .html(`Species: ${d.species}<br />Island: ${d.island}<br />Weight: ${d.MYEAR}kg`);
    })
